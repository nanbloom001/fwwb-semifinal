...
class NavConfig(StageConfig):
    """
    Stage: nav — end-to-end navigation training on track terrain.
    阶段：nav —— track 地形端到端导航训练。

    Policy outputs 12D joint actions directly (same as locomotion).
    Navigation signals come from framework-provided rewards
    (heuristic_navigation / deadend_escape / wall_proximity_brake)
    that use nav_scanner internally, so no observation-dimension change is needed.
    策略直接输出 12D 关节动作，导航信号来自框架自带的 heuristic_navigation
    等 reward（内部使用 nav_scanner），无需改观测维度。
    """

    name = "nav"
    task_type = "track"

    # --- Override training hyperparameters for navigation fine-tuning ---
    lr = 1e-4
    num_learning_epochs = 5
    num_mini_batches = 4
    num_steps_per_env = 48
    min_normalized_std = [0.02, 0.01, 0.02] * 4
